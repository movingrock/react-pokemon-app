...
const DetailPage = () => {
  ...
  // 포켓몬 id를 기반으로 각각의 포켓몬 데이터(이름, 아이디, 무게, 스탯)를 가져옴
  // 이전, 다음 번호의 포켓몬 이름을 객체로 가져옴
  ...
  const filterAndFormatDescription = (flavorText) => {
    const koreanDescriptions = flavorText?.filter((text) => text.language.name === 'ko')
      .map((text) => text.flavor_text.replace(/\r|\n|\f/g, ' '))
    
    return koreanDescriptions
  }

  // 여러 버전중 한 설명만 랜덤으로 출력 ---> 나중에 랜덤이 아니라 다 출력하는걸로 변경
  ...
  // (id - 1)을 한 이유: offset에서부터 한개를 가져온다
  // pokemonData.next가 없다면 nextResponse 는 undefined 된다. 있다면 다음 값을 가져온다.
  // return next는 ?(옵셔널 체이닝)을 통해서 값이 없으면 undefined를 반환한다. 안전하게 접근하는 방법. ?를 안 쓰고 값을 못가져오면 에러가 발생할 수도 있음
  // return next와 previous를 묶은 객체값을 반환한다.
      ...
      >
        ...
        <section className="w-full min-h-[65%] h-full bg-gray-800 z-10 pt-14 flex flex-col items-center gap-3 px-5 pb-4">
          ...
          <div className="flex w-full items-center justify-between max-w-[400px] text-center">
            ...
            <div className="w-full">
              <h4 className="text-[0.5rem] text-zinc-100">Move</h4>
          ...
          <div
            className="flex my-8 flex-wrap justify-center">
